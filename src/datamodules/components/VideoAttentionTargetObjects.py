...
class VideoAttentionTargetObjects(Dataset):
    def __init__(
        self,
        data_dir: str,
        ...
        face_bbox_overflow_coeff: float = 0.1,
        ...
        self.faces_bbox_overflow_coeff = face_bbox_overflow_coeff
        ...
        self._prepare_videoattentiontarget_dataset(use_gaze_inside_only)
    ...
    def _prepare_videoattentiontarget_dataset(self, use_gaze_inside_only: bool):
        ...
        self.keys_videoattentiontarget = list(df.groups.keys())
    ...
    def __get_train_item__(self, index: int):
        ...
        # Load depth
        depth = Image.open(
            os.path.join(self.data_dir, "depths", self.keys_videoattentiontarget[index])
        )
        ...
        gaze_heatmaps = []
        gaze_watch_outside = []
        for _, row in self.X_videoattentiontarget.get_group(
        ...
        if self.keys_videoattentiontarget[index] in self.X_objects_aux.groups:
            # Sort objects by conf
            objects = self.X_objects_aux.get_group(
                self.keys_videoattentiontarget[index]
        ...
        for idx in range(len(target["gaze_points"])):
            ...
            elif self.gaze_vector_type == "3d_angle_magnitude":
    ...
    def __get_test_item__(self, index: int):
        ...
        gaze_vectors_dimensions = 2 if self.gaze_vector_type == "2d" else 3
        gaze_vectors = torch.full(
            (self.num_queries, 20, gaze_vectors_dimensions),
            0,
        ).float()
        gaze_vectors[: len(target["gaze_vectors"]), :] = target["gaze_vectors"]
        target["gaze_vectors"] = gaze_vectors
